...
function inbuilts() {
  function createShadowFunc(object, context, floorWidth, floorHeight, translate, rotate) {
    const translateX = floorWidth / 2 + translate.x;
    const translateY = floorHeight / 2 + translate.z;
    let width = object.width || 60;
    // const height = object.height || 200;
    const depth = object.depth || 80;
    const lineWidth = depth * 0.99;
    width = width * 0.99;
  ...
  function filterJsonFunc(box, json, name) {
    const objects = [];
    const translates = json.translates;
    const rotates = json.rotates || [];
    let clients = json.clients || [];

    if (clients.length < translates.length && clients.length > 0) {
    ...
    if (translates) {
      translates.map((e, i) => {
        ...
        demo.copyProperties(json, device, ['type', 'translate', 'translate', 'rotates', 'rotate', 'client']);
        objects.push(device);
      });
    }
    return objects;
  ...
  function registerCreatorFunc(box, json, name, frontImg, color) {
    const translate = json.translate || [0, 0, 0];
    const rotate = json.rotate || [0, 0, 0];
    let x = translate[0],
      y = translate[1],
      z = translate[2];
    const width = json.width || 60;
    const height = json.height || 180;
    const depth = json.depth || 80;
    const shadow = json.shadow;
    const client = json.client;

    const device = new mono.Cube(width, height, depth);
    device.s({
      'm.color': color,
      ...
      'top.m.normalmap.image': demo.getRes('r1srack.jpg'),
      'left.m.normalmap.image': demo.getRes('r1srack.jpg'),
      'right.m.normalmap.image': demo.getRes('r1srack.jpg'),
      'back.m.normalmap.image': demo.getRes('r1srack.jpg'),
      ...
      'front.m.texture.image': demo.getRes(frontImg),
      'front.m.texture.repeat': new mono.Vec2(1, 1),
      'front.m.specularmap.image': demo.getRes('white.png'),
      'front.m.color': '#888',
      'front.m.ambient': '#888',
      'front.m.specularStrength': 20
    });
    device.setPosition(x, height / 2 + 1 + y, z);
    device.setRotation(rotate[0], rotate[1], rotate[2]);

    device.shadow = shadow;
    device.setClient('type', name);
    device.setClient('origin', device.getPosition().clone());
    device.setClient('loaded', false);

    if (client) {
      Object.keys(client).forEach((e) => {
        device.setClient(e, client[e]);
        if (e === 'name') {
          device.setClient('mousemove', true);
        }
      });
    }
  ...
  function setAllClient(client, element) {
    if (client) {
      Object.keys(client).forEach((e) => {
        element.setClient(e, client[e]);
        if (e === 'name') {
          element.setClient('mousemove', true);
        }
      });
    }
  }

  demo.registerFilter('powerCabinet', (box, json) => {
    return filterJsonFunc(box, json, 'powerCabinet');
  });

  demo.registerCreator('powerCabinet', (box, json) => {
    registerCreatorFunc(box, json, 'powerCabinet', 'h1frack.jpg', '#488aa0');
  });

  demo.registerShadowPainter('powerCabinet', createShadowFunc);
  ...
  demo.registerCreator('rack', function(box, json) {
    ...
    rack.s({
      ...
      'm.type': 'phong',
      'm.specularStrength': 50,
      'front.m.texture.image': demo.getRes('rack42u.png'),
      'front.m.texture.repeat': new mono.Vec2(1, 1),
      'front.m.specularmap.image': demo.getRes('white.png'),
      'front.m.color': '#eee',
      'front.m.ambient': '#eee',
    ...
    var loader = function(box, width, height, depth, severity, rack, json) {
      ...
      setAllClient(client, newRack);

      newRack.oldRack = rack;
      if (rack.realTimeData) {
        newRack.realTimeData = rack.realTimeData;
      }
  ...
  demo.registerCreator('post', function(box, json) {
    ...
    post.s({
      'm.texture.image': demo.getRes(pic),
